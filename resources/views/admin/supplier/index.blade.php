                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex justify-content-start">
                                        ...
                                        <form action="{{ route('admin.suppliers.destroy', $supplier->id) }}" method="POST" id="deleteSupplierForm-{{ $supplier->id }}" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" id="deleteButton-{{ $supplier->id }}" class="btn btn-danger me-1 mb-1">
...
<script>
    document.addEventListener('click', function (event) {
        if (event.target.closest('[id^="deleteButton-"]')) {
            ...
            const formId = button.id.replace('deleteButton-', 'deleteSupplierForm-'); // Ambil ID form terkait
            const form = document.getElementById(formId); // Cari form berdasarkan ID

            Swal.fire({
                title: 'Konfirmasi Hapus',
                text: "Apakah Anda yakin ingin menghapus pemasok ini?",
                icon: 'warning',
