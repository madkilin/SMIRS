            ...
            <div class="row match-height">
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <p class="text-subtitle text-muted">Alokasikan Inventaris/barang sesuai dengan tempat yang sudah
                                ditentukan.</p>
                            <p>Barang Tersedia Di lokasi: {{ $location->name }}</p>
                            <ul>
                                @foreach ($location->inventories->unique('id')->take(10) as $inventory)
                                    <li>{{ $inventory->name }} (Kuantitas:
                                        {{ $location->inventories()->where('inventory_id', $inventory->id)->count() }})</li>
                                @endforeach

                                @if ($location->inventories->unique('id')->count() > 10)
                                    <li>Dan {{ $location->inventories->unique('id')->count() - 10 }} barang lainnya...</li>
                                @endif
                                    ...
                                    <table class="table table-striped" id="inventoryTable">
                                        ...
                                        <tbody id="inventoryRows">
                                            <tr>
                                                <td>
                                                    <div class="form-group">
                                                        <select name="inventories[0][inventory_id]"
                                                            class="form-select inventorySelect" required>
                                                            <option value="" disabled selected>Silakan pilih
                                                                inventaris/barang yang akan dialokasikan</option>
                                                ...
                                                <td>
                                                    <input type="number" name="inventories[0][quantity]"
                                                        class="form-control inventoryQuantity" min="0" required
                                                        placeholder="Banyak Barang">
                                                </td>
                                                <td>
                                                    <button type="button" class="btn btn-danger removeInventory"
                                                        style="display: none;">
